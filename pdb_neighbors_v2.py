...
def pdb_neighbors(pdb_f, pdb_id):
    ...
    for res in center_res:
        ...
        positive_atom_neighbors = [ns.search(a.get_coord(),POSITIVE_BOND_CUTOFF) for a in atoms if a.get_name() in ['NE2','ND1','NZ','NE','NH2','NH1'] ]
...
def main():
    ...
    with lt.open_file(file_suffix='orignal') as w_f:
        for pdb_id,res,neighbors in res_neighbors:
            if len(neighbors) > 0:
                print >> w_f,'{0:<20}{1:<20}{2}'.format(pdb_id,res,' '.join(neighbors))
